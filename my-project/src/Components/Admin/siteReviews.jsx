...
const AdminReviewsPage = () => {
  ...
  const getClientName = (clientId) => {
    ...
    return client ? `${client.client_first_name} ${client.client_last_name}` : "Unknown Client";
  };
        ...
        <div className="p-6 bg-white shadow-lg rounded-lg mb-10 text-black mt-10">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {reviews.length > 0 ? (
                reviews.map((review) => (
                  <div key={review.review_id} className="bg-white p-4 shadow-lg rounded-lg border border-gray-200">
                    <h2 className="text-xl font-semibold mb-2">{getClientName(review.client_id)}</h2>
                    <p className="text-gray-600 mb-4">{review.content}</p>
                    <div className="flex items-center justify-between">
                      <span className="font-semibold text-yellow-500">{`Rating: ${review.rating}`}</span>
                      <span className="text-gray-400 text-sm">{new Date(review.review_date).toLocaleDateString()}</span>
                    </div>
                  </div>
                ))
              ) : (
