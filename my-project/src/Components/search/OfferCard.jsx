      ...
      <div className="boffer-details pl-4 flex flex-col justify-between w-3/4">
        ...
        <div className="btop-section flex justify-between items-center">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-600">({offer.num_reviews} reviews)</span>
          </div>
        ...
        {/* Title */}
        <h3 className="boffer-title text-lg font-bold mt-2">{offer.offer_name}</h3>

        {/* Departure Date */}
        <p className="bdeparture-date text-sm text-gray-600 mt-1">{formattedDepartureDate}</p>
      ...
      <div className="bagency-info flex items-center mt-2 space-x-2">
        <Link to={`/agencyprofile/${offer.agency_id}`}>
            {offer.logo && offer.logo[0] ? (
              // Convert the BLOB to an object URL
              <img
                src={URL.createObjectURL(new Blob([offer.logo[0]], { type: 'image/png' }))}
          ...
          <Link to={`/agencyprofile/${offer.agency_id}`}>
          <span className="text-sm text-gray-800 font-medium">{offer.agency_name}</span>
          </Link>
      </div>
